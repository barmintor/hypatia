...
describe FtkItemAssembler do
  ...
  context "FtkItem assets" do
    before(:all) do
      ...
      @assembler.file_dir = fixtures_path('ftk')
      @assembler.display_derivative_dir = File.join(@assembler.file_dir,'display_derivatives')
...
#        @content_md_no_deriv_doc.xpath("/contentMetadata/resource/file[@id='foofile.txt']/@mimetype").to_s.should eql("application/octet-stream")  # skipping for Hypatia demo
  ...
  context "create_hypatia_ftk_item" do
    before(:all) do
      ...
      import_fixture(@coll_pid)
      @assembler = FtkItemAssembler.new(:collection_pid => @coll_pid)
      @assembler.ftk_report = fixtures_path('ftk', "Gould_FTK_Report.xml")
      @assembler.file_dir = fixtures_path('ftk')
      @assembler.display_derivative_dir = File.join(@assembler.file_dir,"display_derivatives")
      @ff_intermed = FactoryGirl.build(:ftk_file)
  ...
  context "processing a directory" do
    before(:all) do
      ...
      @assembler = FtkItemAssembler.new(:collection_pid => @coll_pid)
      ftk_report = fixtures_path("ftk","Gould_FTK_Report.xml")
      ftk_file_dir = fixtures_path('ftk')
      display_derivative_dir = File.join(ftk_file_dir, "display_derivatives")
...
def build_fixture_disk_objects
  ...
  di_txt_file = fixtures_path('ftk','disk_images','CM5551212.001.txt')
